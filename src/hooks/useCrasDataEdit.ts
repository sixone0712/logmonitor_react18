...
export default function useCrasDataEdit({ type }: useCrasDataEditCreateProps) {
  ...
  } = useQuery<CrasDataManualInfo[], AxiosError>(
    [QUERY_KEY.RULES_CRAS_MANUAL_CREATE_LIST, selectId],
  ...
  const {
    ...
    [QUERY_KEY.RULES_CRAS_MANUAL_JUDGE_LIST, selectId],
  ...
  useQueries([
    {
      ...
      enabled: type === 'create' && !!selectId,
      refetchOnWindowFocus: false,
      onSuccess: (data: CrasDataCreateOption) => {
        dispatch(setCrasCreateFixedOption(data));
      },
    },
    {
      ...
      refetchOnWindowFocus: false,
      enabled: type === 'judge' && !!selectId,
      onSuccess: (data: CrasDataJudgeOption) => {
        dispatch(setCrasJudgeFixedOption(data));
      },
    },
  ]);
